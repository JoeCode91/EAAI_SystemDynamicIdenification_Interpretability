%% Main program for the simulation of the effect of the relative difference
%% in amplitude for both branches of NObSP using SVM for dynamic systems.

% This program evaluate the impact of the relative change in magnitudes of
% the components, i.e., what happens when the magnitude of x1 is larger 
% than the magnitude of x2 and vice versa.

% The effect of the changes in relative amplitude between the signal x1 and
% x2 is evaluated for both nonlinear block-structure. Relative gain values
% ranging from 0.1 to 1 are used, i.e., the amplitude of the signal x1
% varies between 0.1 to 1 times the magnitude of x2. In total, we performed 
% four simulations, two for each system structure, one varying the relative
% amplitude of x1 concerning x2, and varying the relative amplitude of x2 
% concerning x1.
...
CX_n = (0.1:.1:1); % Relative change of amplitud
...
for Sim=1:2% The first simulation evaluate the performance of the Wiener 
    ...
    % Variable space
    Model_Tr = cell(length(CX_n),2,Forend,4);
    M_Err_Tr_Fit = zeros(length(CX_n),2,Forend);
    M_Err_Tr_Proy = zeros(length(CX_n),2,Forend,2);
    M_Err_Te_Fit = zeros(length(CX_n),2,Forend);
    M_Err_Te_Proy = zeros(length(CX_n),2,Forend,2);
    M_Err_Te_Alp = zeros(length(CX_n),2,Forend,2);
    M_Ti_Te = zeros(length(CX_n),2,Forend,2);
    DataStruct=cell(length(CX_n),2,Forend,9);
    DataStruct_Te=cell(length(CX_n),2,Forend,6);

    for Ex=1:2 % Change of amplitud for each branch
        ...
        for Cont=1:length(CX_n) % Relative difference of amplitude
            N=P(n);
            m = M(M_n);
            %% TRAIN STAGE
            for Conti=1:Forend % Number of iterations
                ...
                M_Err_Tr_Fit(Cont,Ex,Conti,1) = error_MAE;
                ...
                [Y,Model_Tr{Cont,Ex,Conti,4}] = NObSP_Tot(input,inputSV,m,MdlGau,0);
                disp(['Calculando Proyecciones y Coef toma : ',num2str(toc)])
                error = (Y1(m:end)-mean(Y1(m:end)))-(Y(:,1)-mean(Y(:,1)));
                M_Err_Tr_Proy(Cont,Ex,Conti,1,1)=sum(abs(error))/(N*abs(max(Y1(m:end))-min(Y1(m:end))));
                error = (Y2(m:end)-mean(Y2(m:end)))-(Y(:,2)-mean(Y(:,2)));
                M_Err_Tr_Proy(Cont,Ex,Conti,2,1)=sum(abs(error))/(N*abs(max(Y2(m:end))-min(Y2(m:end))));
                Model_Tr{Cont,Ex,Conti,3} = Y;
                Model_Tr{Cont,Ex,Conti,1} = MdlGau;
                Model_Tr{Cont,Ex,Conti,2} = inputSV;
            end
            ...
            % Variable space
            MdlGau_T = Model_Tr{Cont,Ex,SM,1};
            XSV = Model_Tr{Cont,Ex,SM,2};
            Alpha = Model_Tr{Cont,Ex,SM,4};
            ForendTe=Forend;
            ...
            for Conti_te=1:ForendTe % Number of iterations
                ...
                M_Err_Te_Fit(Cont,Ex,Conti_te)=sum(abs(error))/(N*abs(max(y_n(m:end))-min(y_n(m:end))));
                % Calculate the error of each contribution by projections
                error = (y_nx(m:end,1)-mean(y_nx(m:end,1)))-(Ypn_proy(:,1)-mean(Ypn_proy(:,1)));
                M_Err_Te_Proy(Cont,Ex,Conti_te,1)=sum(abs(error))/(N*abs(max(y_nx(m:end,1))-min(y_nx(m:end,1))));
                error = (y_nx(m:end,2)-mean(y_nx(m:end,2)))-(Ypn_proy(:,2)-mean(Ypn_proy(:,1)));
                M_Err_Te_Proy(Cont,Ex,Conti_te,2)=sum(abs(error))/(N*abs(max(y_nx(m:end,2))-min(y_nx(m:end,2))));
                % Calculate the error of each contribution by coefficients
                error = (y_nx(m:end,1)-mean(y_nx(m:end,1)))-(Ypn(:,1)-mean(Ypn(:,1)));
                M_Err_Te_Alp(Cont,Ex,Conti_te,1)=sum(abs(error))/(N*abs(max(y_nx(m:end,1))-min(y_nx(m:end,1))));
                error = (y_nx(m:end,2)-mean(y_nx(m:end,2)))-(Ypn(:,2)-mean(Ypn(:,1)));
                M_Err_Te_Alp(Cont,Ex,Conti_te,2)=sum(abs(error))/(N*abs(max(y_nx(m:end,2))-min(y_nx(m:end,2))));
            end

            disp('ERRORS IN TEST:')
            disp('Outfit -> ')
            fprintf('MAE -> %d\n',mean(M_Err_Te_Fit(Cont,Ex,:)))
            disp('PROJECTIONS')
            disp('Cuadratic-> ')
            fprintf('MAE -> %d\n',mean(M_Err_Te_Proy(Cont,Ex,:,1)))
            disp('Sinc with Cubic-> ')
            fprintf('MAE -> %d\n',mean(M_Err_Te_Proy(Cont,Ex,:,2)))
            fprintf('En %d s\n\n',mean(M_Ti_Te(Cont,Ex,:,1)))
            disp('ALPHA COEF')
            disp('Cuadratic-> ')
            fprintf('MAE -> %d\n',mean(M_Err_Te_Alp(Cont,Ex,:,1)))
            disp('Sinc with Cubic-> ')
            fprintf('MAE -> %d\n',mean(M_Err_Te_Alp(Cont,Ex,:,2)))
            fprintf('In %d s\n\n',mean(M_Ti_Te(Cont,Ex,:,2)))
